...
public class MoveBox : MonoBehaviour
{
    ...
    private float boxPushTime = 0f;
    private float boxPushSetTime = 0.25f;
    ...
    {
        boxPushTime = 0f; // 박스에서 떨어질 경우 0초로 초기화
    }

    private void OnCollisionStay2D(Collision2D collision)
    {
        boxPushTime += Time.deltaTime;

        Debug.Log(boxPushTime);
        if (boxPushTime > boxPushSetTime) //박스를 boxPushSetTime만큼 밀어야 밀림
        {
            boxPushTime = 0f;
            if (collision.gameObject.tag == "Player") //Player는 MoveBox를 이동시킴
            {
                if (!isMove) //이미 움직이는 중이라면 밀수 없음
                {
                    ...
                    if (collision.transform.position.x - transform.position.x < 0 && Mathf.Abs(collision.transform.position.y - transform.position.y) < range)
                    {
                        ...
                        {
                            if (col.tag == "FixedBox" || col.tag == "MoveBox" || col.tag == "Wall"|| col.tag == "WaterBalloon" || col.tag == "Player" || col.tag == "Drunk")
                            {
                                isWall = true;
                                return;
                            }
